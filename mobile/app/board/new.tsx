...
export default function NewBoardPostScreen() {
  ...
  const createPost = useCreateBoardPost(userId);
  ...
  const onSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const uploaded: BoardMediaInsert[] = [];
        ...
        if (item.kind === "video") {
          ...
          if (compressed.thumbnailUri) {
            try {
              ...
              console.warn("board thumbnail upload failed", e);
            }
          }
        }
        ...
        <View className="gap-2">
          ...
          {picked.length > 0 ? (
            ...
            <Pressable
              onPress={onPickMedia}
              disabled={submitting}
              className="border border-dashed border-gray-300 rounded-2xl py-6 items-center justify-center gap-2"
            >
